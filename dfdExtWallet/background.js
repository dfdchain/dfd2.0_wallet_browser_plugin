...
const networkList = [
    ...
    {
        chainId: '68a46fbb5048e5bc0586d38f560e50014f9f34f23679a3fb6ceba63255ba1b1b', // local mainnet
        key: 'localhost',
         name: "localhost:8090", 
...
function getNetworkConfig() {
    network = localSave.getItem("apiPrefix") || "wss://node.dfdtw.com"; // 'ws://120.79.93.99:8091';
    ...
    port.onMessage.addListener(function (msg) {
        ...
        if (msg.src === 'contentScript') {       //message from webpage(DApp page)
            if (!msg.data)
                return;
            // TODO
            if (msg.data.method === "dfd_sendTransaction") {
                cacheTx(msg.data);
            }
            else if (msg.data.method === "dfd_call") {
            ...
            else if (msg.data.method === "dfd_getUserAddress") {
                const config = getNetworkConfig();
                const testnetChainId = "2c5729a8f02e0431233528a3db625a7b0f83aa7c9f561d9bd73886d993a57161";
                const addrPrefix = config.chainId === testnetChainId ? 'DFDT' : 'DFD';
                let curAddr = AccAddress;
                if (curAddr.substr(0, 3) !== 'DFDT' && addrPrefix === 'DFDT') {
            ...
            else if (msg.data.method === 'dfd_getConfig') {
                const config = getNetworkConfig();
                port.postMessage({
                    source: sourceName,
                    config: config,
            ...
            else if (msg.data.method === 'dfd_setConfig') {
                // 修改当前钱包的网络配置
        ...
        //**********************************
            ...
            if (!!msg.data.AccAddress) {
                ...
                //if has serial number, then this message is send from dfdpay,
                if (msg.serialNumber) {
...
function cacheTx(txData, pageType) {
    ...
    chrome.windows.create({ 'url': 'dfdWebWallet/ext_index.html#' + urlHash, 'type': 'popup', height: 1024, width: 500 }, function (window) {
    });
}
...
function UnlockFile(keyJson, password) {
    ...
    try {
        ...
        var address = account.getAddressString("DFD");
        ...
        // this catches e thrown by dfd.js!account
...
// used to save the message from dfdpay, key= serialNumber, value=
var messagesFromPage = {};

//listen msg from contentscript (dfdpay -> contentscript -> background)
