...
function processTx(unapprovedTxs) {
    ...
    if (length > 0) {
        ...
        if(typeof(window) !== 'undefined' && window.dfdAppState) {
            var dfdAppState = window.dfdAppState;
            dfdAppState.pushFlashTx(tx);
            return;
        }
        ...
        if (tx.serialNumber) {           //value send by dfdPay is using unit of Wei
