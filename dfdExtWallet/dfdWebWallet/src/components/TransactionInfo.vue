<template>
  <div>
    <div>
      ...
      <div
        class="dfd-panel dfd-tx-details-panel"
        v-if="txInfo && txInfo.operations && txInfo.operations.length>0"
      >
        <div class="-dfd-details-title">Details</div>
        <div class="-tx-details-panel">
          <div
            v-for="([operationType, operation], operationIndex) in txInfo.operations"
            :key="operationIndex"
          >
            <div class="-operation-title">{{getOperationTypeName(operationType) || "Unknown"}}</div>
            <div v-if="getOperationTypeName(operationType)==='transfer'">
              <TransferOpDetail :operation="operation"></TransferOpDetail>
            </div>
            <div v-if="getOperationTypeName(operationType)==='contract_register'">
              <RegisterContractOpDetail
                :operation="operation"
                :txReceipt="txReceipts[operationIndex]"
              ></RegisterContractOpDetail>
            </div>
            <div v-if="getOperationTypeName(operationType)==='contract_invoke'">
              <InvokeContractOpDetail
                :operation="operation"
                :txReceipt="txReceipts[operationIndex]"
              ></InvokeContractOpDetail>
            </div>
            <div v-if="getOperationTypeName(operationType)==='transfer_contract'">
            ...
            <div v-if="getOperationTypeName(operationType)==='foreclose_balance'">
              <SimpleOpDetail :operation="operation"></SimpleOpDetail>
            </div>
            <div v-if="getOperationTypeName(operationType)==='pay_back'">
              <SimpleOpDetail :operation="operation"></SimpleOpDetail>
            </div>
            <div v-if="getOperationTypeName(operationType)==='account_create'">
              <SimpleOpDetail :operation="operation"></SimpleOpDetail>
            </div>
            <div v-if="getOperationTypeName(operationType)==='crosschain_withdraw_result'">
              <SimpleOpDetail :operation="operation"></SimpleOpDetail>
            </div>
            <div v-if="getOperationTypeName(operationType)==='crosschain_record'">
              <SimpleOpDetail :operation="operation"></SimpleOpDetail>
            </div>
            <div
              v-if="['transfer','contract_register','contract_invoke','transfer_contract',
                'lockbalance','foreclose_balance','pay_back', 'account_create', 
                'crosschain_withdraw_result', 'crosschain_record'].indexOf(getOperationTypeName(operationType))<0"
            >
              <SimpleOpDetail :operation="operation"></SimpleOpDetail>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    dfdAmountToString(val) {
      return utils.dfdAmountToString(val);
    },
            ...
            .then(tx => {
              return nodeClient
                ...
                .then(receipts => {
                  ...
                  console.log("txReceipts: ", this.txReceipts);
                  return tx;
                });
            })
            .then(tx => {
