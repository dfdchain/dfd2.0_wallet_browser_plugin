<template>
  <div>
    <div v-if="!checkTxDone" class="dfd-main-container dfd-check-tx-container">
...
export default {
  ...
  methods: {
    ...
    toQueryTx(txidOrAddress) {
      ...
      let isDfdAccountAddr =
        txidOrAddress.indexOf('D') === 0 &&
        txidOrAddress.length > 20;
      let isDfdContractAddr =
        txidOrAddress.indexOf('c') === 0 && txidOrAddress.length > 20;
      this.checkTxForm.isTxId = isTxId;
      this.checkTxForm.isDfdAccountAddr = isDfdAccountAddr;
      this.checkTxForm.isDfdContractAddr = isDfdContractAddr;
      ...
      appState
        ...
        .then(() => {
          ...
          } else if (isDfdAccountAddr) {
            return nodeClient.getAccountByAddresss(
              txidOrAddress
            );
          } else if (isDfdContractAddr) {
        ...
        .then(data => {
          ...
          if (!isTxId && !isDfdAccountAddr && !isDfdContractAddr) {
            isDfdAccountAddr = true;
            this.checkTxForm.isDfdAccountAddr = isDfdAccountAddr;
          }
          return data;
        })
...
<style lang="less">
.dfd-check-tx-container1 {
  .label-font {
    color: #a99eb4;
    font-size: 8pt;
  }
}
.dfd-check-tx-container2 {
  .label-font {
    color: #a99eb4;
    font-size: 8pt;
  }
}
.dfd-check-tx-container,
.dfd-check-tx-done-container {
  min-width: 400px;
  min-height: 266pt;
  .-check-tx-title {
    font-size: 20pt;
