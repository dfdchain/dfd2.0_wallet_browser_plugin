<template>
  <div>
    <div class="dfd-panel dfd-invoke-contract-container">
      <div class="-left-side" v-if="step==='transfer'">
        <el-form
          ...
          ref="contractForm"
          label-width="60pt"
          class="dfd-invoke-contract-inner-container"
        >
          <el-form-item
          ...
          >
            ...
            <el-button
              type="primary"
              class="dfdwallet-form-btn -confirm-contract-address-btn"
          ...
          <el-form-item v-bind:label="$t('contractPage.balance')" prop="amount">
            <el-input
              ...
              type="text"
              :readonly="true"
              v-model="currentAccountDfdBalance"
              style
            >
              <template slot="append">DFD</template>
            </el-input>
          </el-form-item>
          ...
          <el-form-item v-if="emulateState==='success'">
            <div class="-invoke-contract-api-result">API result: {{contractForm.apiResult}}</div>
          </el-form-item>
        </el-form>
        <el-dialog
      ...
      <div class="-left-side" v-if="step==='contract_sent'">
        ...
        <div style="margin-top: 10pt; font-size: 35pt; color: #308ce5;">
        ...
        <div>
          <el-button
            type="text"
            style="color: #308ce5; font-size: 10pt;"
            @click="toViewTx(lastSentTxId)"
          >{{lastSentTxId}}</el-button>
        </div>
        <div style="margin-top: 15pt;">
          <el-button class="dfdwallet-form-btn" @click="backToTransfer">{{$t('contractPage.return')}}</el-button>
        </div>
      </div>
  ...
  methods: {
          ...
          return nodeClient
            ...
            .then(balances => {
              ...
              for (let asset of assets) {
                ...
                if (asset.id === "1.3.0") {
                  this.currentAccountDfdBalance = item.amountNu.toFixed(
                    asset.precision
                  );
                }
              }
    ...
    doSubmitInvokeContract: _.throttle(function () {
      ...
      const gasPrice = parseInt(
        gasPriceNu.multipliedBy(Math.pow(10, appState.dfdPrecision)).toFixed(0)
      );
...
.dfd-invoke-contract-container {
  ...
  .dfd-address-or-select-wallet-input {
    .-address-show-label {
...
.chrome-ext-app-container .dfd-invoke-contract-container {
  .-input-gas-price.asset-input-wrapper {
    .el-input {
...
.dfd-invoke-contract-inner-container,
.dfd-invoke-contract-done-inner-container {
  min-width: 500pt;
