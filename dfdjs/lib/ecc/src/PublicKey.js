...
// const addressPrefix = ChainConfig.address_prefix;
...
class PublicKey {
    ...
    toString(address_prefix) {
        // const addressPrefix = ChainConfig.address_prefix;
        // address_prefix = address_prefix || addressPrefix;
    ...
    */
    toPublicKeyString(address_prefix) {
        // const addressPrefix = ChainConfig.address_prefix;
        // address_prefix = address_prefix || addressPrefix;
        // const pubkeyPrefix = address_prefix === "DFDT" ? "DFD" : address_prefix;
        var pub_buf = this.toBuffer();
        var checksum = ripemd160(pub_buf);
        var addy = Buffer.concat([pub_buf, checksum.slice(0, 4)]);
        return 'D' + encode(addy);
    }

    /**
        @arg {string} public_key - like Dyz...
    ...
    */
    ...
    /**
        @arg {string} public_key - like DFDyz...
        @arg {string} address_prefix - like GPH
    ...
    */
    static fromStringOrThrow(public_key, address_prefix) {
        const addressPrefix = ChainConfig.address_prefix;
        address_prefix = address_prefix || addressPrefix;
        const pubkeyPrefix = 'D';
        if (public_key.Q === null)
            public_key =
                pubkeyPrefix + "1111111111111111111111111111111114T1Anm"; // null key
        var prefix = public_key.slice(0, pubkeyPrefix.length);
        assert.equal(
            pubkeyPrefix,
            prefix,
            `Expecting key to begin with ${pubkeyPrefix}, instead got ${prefix}`
        );
        public_key = public_key.slice(pubkeyPrefix.length);
        ...
        if (!isEqual) {
            throw new Error(
                "Checksum did not match when prefix is " + pubkeyPrefix
            );
        }
        return PublicKey.fromBuffer(public_key);
    }

    toAddressString(address_prefix, version = 30) {
        // const addressPrefix = ChainConfig.address_prefix;
        // address_prefix = address_prefix || addressPrefix;
        var pub_buf = this.toBuffer();
        var pub_sha = sha256(pub_buf);
        var addy = ripemd160(pub_sha);
        var binAddy = Buffer.concat([Buffer.from([version]), addy]);
        var checksum = sha256(sha256(binAddy));
        binAddy = Buffer.concat([binAddy, checksum.slice(0, 4)]);
        return encode(binAddy);
    }
